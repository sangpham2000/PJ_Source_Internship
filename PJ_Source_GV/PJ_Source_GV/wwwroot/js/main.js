$(function () {
    ...
    //BEGIN LEFT MENU
    var item = $('#sidebar a').filter(function () { return location.pathname.toLowerCase().includes($(this).attr("href").toLowerCase()); }).last();

    item.parent().addClass("active");
    if (!item.parent().parent('ul.nav').parent().hasClass("active")) {
...
$(function () {
    ...
    $('.selectpicker').data('selectpicker').$menu.on('click', 'li a', function (e) {
        ...
        var selectOnlyOneOptionByGroup = $(this).hasClass('selection-by-group');

        // Only override the one declared in the html with dummy class "exadra37-1-selection-by-group"
        if (selectOnlyOneOptionByGroup) {

            var isClickTriggeredClass = $(this).hasClass('click-triggered');
            ...
            if (!isClickTriggeredClass) {
                ...
                $.each(lisSelected, function () {
                    ...
                    if (optionGroupToTriggerClass == optionGroupClickedClass) {
                        ...
                        $(this).children('a').addClass('click-triggered').trigger('click');
                    };
                });
                ...
                $(this).removeClass('click-triggered');
            }
        }
    });
});